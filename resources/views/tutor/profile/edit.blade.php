            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-slate-100 p-8">
                <div class="mb-8 border-b border-slate-100 pb-4 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 mt-1">Keep your credentials and rates up to date for prospective students.</p>
                    </div>
                ...
                <form method="POST" action="{{ route('tutor.profile.update') }}" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="bio" name="bio" rows="6" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl shadow-sm block mt-1 w-full text-sm" placeholder="Write a detailed introduction about yourself, teaching styles, and expertise..." required>{{ old('bio', $profile->bio) }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-4 mt-2">
                                @if ($profile->profile_photo)
                                    <img src="{{ Storage::url($profile->profile_photo) }}" alt="Current Photo" class="w-16 h-16 rounded-full object-cover border border-slate-200 shadow-sm">
                                @else

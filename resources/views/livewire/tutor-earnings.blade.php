<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 relative" wire:loading.class="opacity-50 transition-opacity">
        <div class="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-8 shadow-md text-white flex items-center relative overflow-hidden">
            ...
            <div class="z-10">
                <p class="text-emerald-100 font-medium uppercase tracking-wider text-sm">Earnings <span wire:loading class="ml-2 text-xs text-white">Updating...</span></p>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mt-8">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-bold text-slate-800">Your Earnings Trend</h3>
        </div>
        <div class="relative h-80 w-full flex justify-center" wire:ignore>
            <canvas id="tutorEarningsChart"></canvas>
        </div>
        @if(array_sum($chartData) == 0)
            <div class="flex items-center justify-center h-full text-slate-400 text-sm mt-4">Not enough activity yet to generate insights.</div>
        @endif
    </div>
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($recentEarnings as $earning)
                        <tr wire:key="earning-{{ $earning->id }}" class="hover:bg-slate-50 transition-colors">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-slate-500">
                                You haven't completed any paid sessions yet in this timeframe.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <script>
        document.addEventListener('livewire:init', function() {
            let tutorEarningsChart;
            const ctx = document.getElementById('tutorEarningsChart');
            
            if (ctx) {
                tutorEarningsChart = new Chart(ctx, {
                    type: 'bar',
                    data: {
            ...
            Livewire.on('tutor-chart-updated', (data) => {
                const eventData = data[0];
                if (tutorEarningsChart) {
                    tutorEarningsChart.data.labels = eventData.chartLabels;
                    tutorEarningsChart.data.datasets[0].data = eventData.chartData;
                    tutorEarningsChart.update();
                }
            });
        });
    </script>
